...
function! s:MatchColorValue(color, pattern, part)
  ...
  if !exists("w:matchescache[a:part]")
    let w:matchescache[a:part] = matchadd(group, a:part)
  endif
...
function! s:ClearMatches()
  if !exists('w:matchescache')
    return
  endif
  for i in values(w:matchescache)
    call matchdelete(i)
  endfor
  unlet w:matchescache
endfunction
...
function! s:PreviewCSSColorInLine(w)
  let line=getline(a:w)
  let colorexps = {
    \ 'hex3' : '#\(\x\)\(\x\)\(\x\)\>', 
    \ 'hex'  : '#\(\x\{6}\)\>',
  ...
  for exp in keys(colorexps)
      ...
      while 1
          let foundcolor = matchstr(line, colorexps[exp], place)
          let place = matchend(line, colorexps[exp], place)
